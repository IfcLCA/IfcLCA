...
const projectSchema = new mongoose.Schema({
  ...
  phase: String,
  description: String,
  customImage: String, // URL to an image
...
projectSchema.methods.calculateTotalCarbonFootprint = async function () {
  try {
    ...
    const buildingElements = await BuildingElement.find({
      ...
      path: "materials_info.material", // materials_info is an array and has a material reference
    ...
    buildingElements.forEach((element) => {
      element.materials_info.forEach((materialInfo) => {
        ...
        const co2Indicator = parseFloat(
          materialInfo.material["Treibhausgasemissionen, Total [kg CO2-eq]"]
        );
